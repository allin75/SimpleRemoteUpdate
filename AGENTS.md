...
- Current `projects[]` deployment-related extensions include `allow_initial_deploy`, `service_install_mode`, `service_exe_path`, `service_args`, `service_display_name`, `service_description`, `service_start_type`, `service_restart_enabled`, `service_restart_cron`, and `runtime_log_dir`; keep docs and UI/API payloads aligned when changing them. System-level service wrapper extension `nssm_exe_path` must also stay aligned across docs and UI/API payloads when changed.
